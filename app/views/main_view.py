"""
Janela principal da aplicação (camada View do MVC).

Exibe lista de compromissos, filtros de período e botões de ação.
Não acessa o calendário diretamente — apenas chama callbacks registrados pelo Controller.
"""
...
# Paleta de cores do tema escuro (usada também em outros módulos da view)
COLORS = {
    "bg": "#0f1419",           # Fundo da janela
    "surface": "#1a2332",      # Painéis e cabeçalho
    "surface_hover": "#243044",
    "accent": "#3b82f6",       # Azul de destaque (botões, seleção)
    "accent_hover": "#2563eb",
    "danger": "#ef4444",       # Botão excluir
    "danger_hover": "#dc2626",
    "text": "#f1f5f9",
    "text_muted": "#94a3b8",
    "border": "#334155",
    "card_selected": "#1e3a5f",  # Card selecionado na lista
}


class MainView(ctk.CTk):
    """
    Janela principal herdando CTk (CustomTkinter).

    Guarda referências aos callbacks do controlador e ao compromisso selecionado.
    """
    ...
    def __init__(self) -> None:
        ...
        self._selected_entry_id: str | None = None  # Id do card clicado
        self._appointment_cards: dict[str, ctk.CTkFrame] = {}  # entry_id -> widget card

        # Funções que o CalendarController vai preencher em bind_handlers
    ...
    def bind_handlers(
        ...
        """
        Registra funções do controlador para cada ação da interface.

        A view só chama essas funções; não sabe como o calendário funciona.
        """
    ...
    def get_date_range(self) -> tuple[date, date]:
        """
        Calcula intervalo de datas conforme rádio selecionado (7, 30, 90 ou 365 dias).

        Sempre começa em hoje.
        """
    ...
    def show_appointments(self, appointments: list[Appointment]) -> None:
        """
        Limpa a lista e desenha um card por compromisso.

        Chamado pelo controlador após list_appointments no repositório.
        """
    ...
    def show_error(self, message: str) -> None:
        """Atualiza barra inferior com texto de erro em vermelho."""
        self._status_label.configure(text=f"Erro: {message}", text_color=COLORS["danger"])

    def show_info(self, message: str) -> None:
        """Atualiza barra inferior com mensagem informativa."""
    ...
    def select_appointment(self, entry_id: str) -> None:
        """Destaca visualmente o compromisso (ex.: após criar um novo)."""
        if entry_id in self._appointment_cards:
            self._select_card(entry_id)

    def _build_layout(self) -> None:
        """
        Monta toda a interface: cabeçalho, barra lateral, lista e rodapé.

        Estrutura:
          [ Cabeçalho: título + Novo | Editar | Excluir ]
          [ Sidebar: período ] [ Lista rolável de cards ]
          [ Rodapé: status ]
        """
    ...
    def _add_appointment_card(self, appt: Appointment) -> None:
        """Cria um card visual na lista para um compromisso."""
        card = ctk.CTkFrame(
    ...
    def _bind_card_selection(self, widget: ctk.CTkBaseClass, entry_id: str) -> None:
        """
        Vincula clique em todo o card e filhos (labels não propagam sozinhos).

        Clique simples: seleciona. Duplo clique: seleciona e abre edição.
        """
    ...
    def _select_card(self, entry_id: str) -> None:
        """Marca um card como selecionado e habilita Editar/Excluir."""
    ...
    def _update_action_buttons(self) -> None:
        """Editar e Excluir só ficam ativos quando há um card selecionado."""
